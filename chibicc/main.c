...
typedef enum { FILE_NONE, FILE_C, FILE_ASM, FILE_OBJ, FILE_AR, FILE_DSO,
} FileType;
...
static bool take_arg(char *arg) {
    char *x[] = {"-o", "-I", "-idirafter", "-include", "-x", "-MF", "-MT", "-Xlinker",
    };
...
static bool in_stc_include_path(char *path) {
...
static void print_dependencies(void) {
  ...
  for (int i = 0; files[i]; i++) {
    if (opt_MMD && in_stc_include_path(files[i]->name))
        continue;        
    fprintf(out, " \\\n  %s", files[i]->name);
  }
  fprintf(out, "\n\n");
...
static void cc1(void) {
  ...
  tok = preprocess(tok);
    ...
    // If -M of -MD are given, print file dependencies.
    if (opt_M || opt_MD) {
        print_dependencies();
        if (opt_M)
            return;
